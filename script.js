        ...
        function testarHierarquia() {
            // Exemplo da classe 1 - Banco
            console.log("Testando hierarquia da classe 1:");
            
            // Contas da classe 1 relacionadas a bancos
            const contasBanco = dadosPGC.filter(conta => 
                conta.CLASSE === "1" && 
                conta.CODIGO.startsWith("12")
            ).sort((a, b) => parseInt(a.CODIGO) - parseInt(b.CODIGO));
            
            contasBanco.forEach(conta => {
                console.log(`  Código: ${conta.CODIGO} - ${conta.CONTA} (Grau: ${conta.GRAU})`);
            });
            
            // Verificar se 121 vem antes de 1211
            const indices = contasBanco.map(c => c.CODIGO);
            const index121 = indices.indexOf("121");
            const index1211 = indices.indexOf("1211");
            
            if (index121 < index1211) {
                console.log("✓ Hierarquia correta: 121 vem antes de 1211");
            } else {
                console.log("✗ Hierarquia incorreta: 121 não vem antes de 1211");
            }
        }
        ...
        fetch('pgc.json')
            ...
            .then(data => {
                ...
                // Executar teste de hierarquia
                setTimeout(testarHierarquia, 100);
            })
            .catch(err => {
